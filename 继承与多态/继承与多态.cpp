...
namespace gl07
{
	/*
	如何解释多态？
	静态（编译时期）的多态： 函数重载[函数重载不通过返回值区分]、模板（函数模板和类模板）
	...
	*/
	...
	/*
	下面的一组bark API接口(3个void bark{...})无法做到我们软件涉及要求的“开-闭“原则
	软件设计由六大原则   “开-闭“原则 --> 对修改关闭，对扩展开放
	
	删除Dog类 需要相应删除对应的 void bark(Dog &dog){...}
	*/
	void bark(Animal* p)
	{
		p->bark(); // Animal::bark虚函数，动态绑定了 
		/*
		p->cat 访问 Cat 的 vftable 中的 &Cat::bark
...
namespace gl02 {
	...
	解答：在派生类的初始化列表中通过调用基类相应的构造函数来初始化
...
namespace gl03 {
	...
	*/
	...
	int main03()
	{
		...
		Derive d;
		d.show();
		d.Base::show();//调用基类的 调用时加作用域
		// 报错：“Derive::show”: 函数不接受 1 个参数
...
namespace gl04
{
	...
	*/
	...
	int main04()
	{
		...
		pb->Base Base::show 如果发现show是 virtual 函数，就进行动态绑定了
		...
		*/
		...
		cout << sizeof(Base) << endl; // 4 8 分别是不带/带虚函数
		cout << sizeof(Derive) << endl; // 8 12 分别是不带/带虚函数
		...
		*/
		cout << typeid(*pb).name() << endl; // class Base 虚函数输出 class Derive 
		/*
		输出：
...
namespace gl05 {
	...
	static静态成员方法 virtual + static NO！
	...
	*/
	...
	protected:
		int ma;
	}; // 虚表 &Base::~Base   &Base::show
	class Derive : public Base // 虚表 &Derive::~Derive   &Base::show 
	{
	public:
		Derive(int data)
...
namespace gl06 {
	...
	*/
	...
	int main06()
	{
		...
		// 动态绑定（通过指针或者引用变量调用虚函数，才发生动态绑定）
		Derive* pd1 = &d;
		pd1->show();
		Derive& rd1 = d;
		rd1.show();

		Derive* pd2 = (Derive*)&b; // 危险
