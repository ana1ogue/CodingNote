...
const怎么理解？ 
const修饰的变量不能够再作为左值！！！因为初始化完成后，值不能被修改！！！

C和C++中const的区别是什么？
const的编译方式不同
C中，const就是当作一个 变量 来编译生成指令的。并且可以通过指针修改
C++中，所有出现const常量名字的地方，都被常量的初始化替换了！！！

c++的const
必须初始化的，叫常量

const int a = b a叫常变量   因为初始值不是立即数，是一个变量
*/
...
extern "C"{
	int sum(int a, int b) // C++ 中 sum_int_int  .text 
	{
		return a + b;
	}
}
...
int main04()
{
	...
	// const如果右边没有指针*的话，const是不参与类型的，const修饰的是q2
...
const修饰的量常出现的错误是：
1.常量不能再作为左值  《=  直接修改常量的值(试图修改常量的值)
2.不能把常量的地址泄露给一个普通的指针或者普通的引用变量 《= 可以间接修改常量的值

const和一级指针的结合：有两种情况
C++的语言规范：const修饰的是离它最近的类型 类型(int)
...
1. malloc开辟内存失败，是通过返回值和nullptr做比较；
2. 而new开辟内存失败，是通过抛出bad_alloc类型的异常来判断的。
*/
int main()
{
	// new有多少种？
	int *p1 = new int(20); // 平凡new

	int *p2 = new (nothrow) int; // 不抛异常的new

	const int *p3 = new const int(40);

	// 定位new  
	int data = 0;
	int *p4 = new (&data) int(50); // 在指定内存的位置划分出来
	cout << "data:" << data << endl;
