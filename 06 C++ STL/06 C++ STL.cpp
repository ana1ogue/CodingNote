...
namespace gl06
{
	...
	int main07()
	{
		...
		// stuMap.erase(it) stuMap.erase(1020)  stuMap[2000] [2000, V()] 默认构造函数
	...
	*/
	int main05()
	{
		...
		*/
		...
		map的operator[]
		1. 查询
		2. 副作用:如果key不存在，它会插入一对数据[key, string()] value的默认值
...
{
	/*
	标准容器 - 容器适配器 => 设计模式，就叫做适配器模式(代理模式 stack提供的方法把底层容器的方法代理了一下)
	怎么理解这个适配器？
	...
	二. priority_queue => vector 为什么依赖vector？？？
		1. 底层默认把数据组成一个大根堆结构  在一个内存连续的数组上构建一个大根堆或者小根堆的
		2. 按照下标区分左右孩子 deque第二维不是连续的，其是分段连续的
	*/
	int main04()
	{
...
{
	/*
	vector特点：动态开辟的数组，内存是连续的，2倍的方式进行扩容， vector<int> vec; 0-1-2-4-8... reserve(20)/resize
	deque特点：动态开辟的二维数组空间，第二维是固定长度的数组空间，扩容的时候（第一维的数组进行2倍扩容）
			面经问题：deque底层内存是否是连续的？   并不是  每一个第二维是连续的 也就是分段连续的
...
namespace gl02
{
	...
	mylist.insert(it, 20); it指向的位置添加元素 O(1) // 往往在链表中进行insert的时候，先要进行一个query查询操作
		对于链表来说，查询操作效率就比较慢了
...
namespace gl01
{
	...
	*/
	int main01()
	{
		vector<int> vec; // vector<string> vec; 0 1 2 4 8 16 32 64扩容
		...
		while (it2 != vec.end())
		{
			if (*it2 % 2 == 0)
			{
				it2 = vec.erase(it2); // erase会返回当前元素的迭代器
			}
			else
			{
				++it2;
